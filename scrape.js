...
(async () => {
    ...
    await wait(2000);
    await page.$eval("select#gakkiKubunCode", select => { select.value = "5"}); // S1, S2, A1, A2 の順で 3 ~ 6
    await wait(2000);
    ...
    var basicInfos = await page.$$eval("tbody tr", rows => rows.map(row => {
    ...
    await page.$eval("select#gakubuShozokuCode", select => { select.value = "00"});
    await wait(2000);
    await page.$eval("select#gakkiKubunCode", select => { select.value = "6"}); // S1, S2, A1, A2 の順で 3 ~ 6
    await wait(2000);
    await page.$eval("select[name=_displayCount] option[selected]", option => option.value = "5000");
    await wait(1000);
    await page.$x("//input[contains(@value, '検索')]").then(([searchButton]) => searchButton.click());
    ...
    basicInfos = basicInfos.concat(await page.$$eval("tbody tr", rows => rows.map(row => {
        const [, semester, _periods,,, classroom,, code, _type, category, titleJp, lecturerJp,] = [...row.children].map(item => item.innerText.trim());
        const periods = _periods.replace(/\s/, "").split(",").map(period => period.trim());
        const type = _type.replace(/(.+)科目/, "$1");
    ...
    for (const [i, basicInfo] of basicInfos.entries()) {
        ...
        await page.goto(getDetailsUrl(basicInfo.code, flowExecutionKey, (i < num_of_A1)?"5" :"6"));
        await wait(2500);
        ...
        if (i % 50 === 0) {
            ...
            await page.$eval("select#gakubuShozokuCode", select => { select.value = "00"});
            await wait(2000);
            await page.$eval("select#gakkiKubunCode", (select, i, num_of_A1) => { select.value = (i < num_of_A1) ? "5" : "6";}, i, num_of_A1); // S1, S2, A1, A2 の順で 3 ~ 6
            await wait(2000);
            await page.$eval("select[name=_displayCount] option[selected]", option => option.value = "5000");
            await wait(1000);
            await page.$x("//input[contains(@value, '検索')]").then(([searchButton]) => searchButton.click());
